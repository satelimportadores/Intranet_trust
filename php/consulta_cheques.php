		...
		$Rbancos = $con->query($query) or trigger_error($con->error);
			while ($row = $Rbancos->fetch_array()) {
				if ($row['dias'] < 10 ) { //el rango de dias para mostrar cheques por defecto proximos 10 dias
					...
					<div class="col-md-2"><a href='archivos/<?php echo $row['adjunto'] ?>'>Adjunto</a></div>
					<div class="col-md-2"><button data-codigo-id="<?php echo $row['id'] ?>" data-cheque="<?php echo $row['numero_cheque']?>" data-operacion="consig" data-toggle="modal" data-target="#myModal" >Consignado</button></div>
					<?php
				}
			}
		}
		...
		$Rbancos = $con->query($query) or trigger_error($con->error);
			while ($row = $Rbancos->fetch_array()) {
				if ($row['dias'] < 10 ) { //el rango de dias para mostrar cheques por defecto proximos 10 dias
					...
					<div class="col-md-2"><b>fecha </b><?php echo $row['fecha_cheque'] ?></div>
					<div class="col-md-2"><a href='archivos/<?php echo $row['adjunto'] ?>'>Adjunto</a></div>
					<div class="col-md-1"><button data-codigo-id="<?php echo $row['id'] ?>" data-cheque="<?php echo $row['numero_cheque']?>" data-operacion="devuelt" data-toggle="modal" data-target="#myModal" >Devuelto</button></div>
					<div class="col-md-1"><button data-codigo-id="<?php echo $row['id'] ?>" data-cheque="<?php echo $row['numero_cheque']?>" data-operacion="efect" data-toggle="modal" data-target="#myModal" >Efectivo</button></div>
					<?php
				}
			}
		}
		...
		$Rbancos = $con->query($query) or trigger_error($con->error);
			while ($row = $Rbancos->fetch_array()) {
				if ($row['dias'] < 10 ) { //el rango de dias para mostrar cheques por defecto proximos 10 dias
					...
					<div class="col-md-2"><b>fecha </b><?php echo $row['fecha_cheque'] ?></div>
					<div class="col-md-2"><a href='archivos/<?php echo $row['adjunto'] ?>'>Adjunto</a></div>
					<div class="col-md-2"><button data-codigo-id="<?php echo $row['id'] ?>" data-cheque="<?php echo $row['numero_cheque']?>" data-operacion="cambio" data-toggle="modal" data-fecha="<?php echo $row['fecha_cheque'] ?>" data-target="#myModal" >Cambiar Fecha</button></div>
					<?php
				}
			}
		}
